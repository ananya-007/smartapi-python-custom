...
class SmartConnect(object):
    ...
    def _request(self, route, method, parameters=None):
        ...
        try:
            r = requests.request(method,
                                 url,
                                 data=json.dumps(params) if method in [
                                     "POST", "PUT"] else None,
                                 params=json.dumps(params) if method in [
    ...
    def convertPosition(self, positionParams):
        ...
        for k in list(params.keys()):
            if params[k] is None:
                del(params[k])
        convertPositionResponse = self._postRequest(
            "api.convert.position", params)
    ...
    def gttLists(self, status, page, count):
        if type(status) == list:
            ...
            message = "The status param is entered as" + \
                str(type(status)) + \
    ...
    def getCandleData(self, historicDataParams):
        ...
        getCandleDataResponse = self._postRequest(
            "api.candle.data", historicDataParams)
